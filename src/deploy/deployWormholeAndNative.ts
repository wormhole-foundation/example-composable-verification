...
(async () => {
  ...
  const ethFactory = new WormholeAndNativeSender__factory(ethSigner);
  ...
  const optFactory = new WormholeAndNativeReceiver__factory(optSigner);
  ...
  log = ora("Registering Receiver address on Sender").start();
  const ethTx = await ethContract.nativeBridgeSetup(optContract.address);
  await ethTx.wait();
  log.succeed();
  console.log(`${ETH_SCAN}/tx/${ethTx.hash}\n`);
